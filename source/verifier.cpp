...
auto verifier::verify(const std::string email) -> awaitable<vrf_data> {
    ...
    const auto mx_records = co_await resolv_.query<dns::qtype::MX>(domain);
    const auto mx_record = co_await resolv_.query<dns::qtype::A>(mx_records.at(0).rdata.mx);
    ...
    auto buf = streambuf{};
    co_await async_read_until(socket_, buf, boost::regex{"220.*$"}, use_awaitable);
